...
class Work:
  ...
  def isUserCreated(self, first_name = False, second_name = False, user_id = False):
    if not first_name and not second_name and not user_id:
      return 'write something of arguments'
    for k in self.get_data():
...
# vk_id, first_name, last_name, increaseEarning, Coins
        return data[1], data[3], data[4], data[5], data[6]
      elif first_name == 'all':
        # list  
        return sht
      else: 
        if first_name == data[3]:
          # tuple
          return sht[index]
  ...
  def update_dataBase(self, user_id, kCoins = False, increaseEarning = False, set_core = False, everyDay = False):

    # user_id - to indentify user
    ...
    if kCoins and (kCoins != ''):
      ...
      if isRunning:
        for index, k in enumerate(self.get_data()):
          if int(user_id) == int(k[1]):
            if everyDay:
              self.cursor.execute('UPDATE users SET coin = %s WHERE vk_id = %s', (kCoins, user_id)); self.connect.commit()
            else:
              self.cursor.execute('UPDATE users SET coin = %s WHERE vk_id = %s', (str(int(self.get_data()[index][6]) + int(kCoins)), user_id)); self.connect.commit()
              return (str(int(self.get_data()[index][6])))

    # elif swap_name and (swap_name != ''):
    #   if self.isUserCreated(user_id = user_id):
    #     for k in self.get_data():
    #       if int(user_id) == int(k[1]):
    #         cursor.execute('UPDATE Work SET name = ? WHERE name = ?', (str(swap_name), str(k[3])))
      ...
      if isRunning:
        for index, k in enumerate(self.get_data()):
          if user_id == k[1]:
            self.cursor.execute('UPDATE users SET increaseEarning = %s WHERE vk_id = %s', (str(int(self.get_data()[index][5]) + int(increaseEarning)), user_id)); self.connect.commit()
            return (str(int(self.get_data()[index][6])))

    elif (set_core != ''):
      try: isRunning = isinstance(int(set_core), int)
      except: raise Exception("You are usersing not with integer..")
      if isRunning:
        for index, k in enumerate(self.get_data()):
          if int(user_id) == int(k[1]):
            self.cursor.execute('UPDATE users SET is_core = %s WHERE vk_id = %s', (set_core, user_id)); self.connect.commit()
            return
    
    else:
      return 'Comon boy, that is not funny, write values!'
  ...
  def pushing(self, first_name, second_name, user_id = False, user_coins = False, increaseEarning = False, error = False, is_core = 0):
    #assert error, 'User already created'

    if error:
      raise Exception('User already created')


    #assert not error, 'User already created'
    ...
    if not len(str(first_name)) and not len(str(second_name)):
      return 'Print user Name!'

    if self.isUserCreated(first_name, second_name, user_id)[0]:
    ...
    if is_core > 1:
      raise Exception('Are you kidding? what do you mean about {} value?'.format(is_core))

    # main adding
    try:
      self.cursor.execute(query, (user_id, 0, first_name, second_name, increaseEarning, user_coins, 0, 0, is_core, 0, 0, 0, 0))
      self.connect.commit()
...
def appending():
  ...
  is_user_core    = int(input("User is core? 1/0: "))

  print(item.pushing(first_value, second_value, user_id, user_coins, increaseEarning, is_core = is_user_core))

def updating():
  first_name1     = input("Enter user_id: ")
  kCoins          = input("How many coins <You can pass this>: ")
  increaseEarning = input("Enter count of increaseEarning <this also>: ")
  set_core        = input("Enter core or not 1/0: ")
  
  response = (item.update_dataBase(user_id = first_name1, kCoins = kCoins, increaseEarning = increaseEarning, set_core = set_core))
  if response is None:
    return
  print(response)
